...
| OrisDent Q (OrisLine) | `dentalpin-vs-orisdent` | `orisline.com`. Publishes a whole product family (OrisDent Q, the cloud OrisDent Air, OrisLab Q for labs). A suite sold as one is the closest thing Italy has to our module argument, so meet it head-on. **The footer of orisline.com reads "Henry Schein ONE Italia srl" (consulted 6 August 2026), so OrisDent shares a parent with Dentrix, Dentally and Software of Excellence in A3. Unlike the Julie row, the vendor publishes this itself.** |
